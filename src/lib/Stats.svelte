...
<svg xmlns="http://www.w3.org/2000/svg" width={size} height={size}>
	<rect x="0" y="0" width={size} height={size} fill="#11111b" rx="12" ry="12" />
	...
	<defs>
		{#each languages as lang, i (lang.name)}
			...
			>
				<stop
					offset="0%"
					stop-color={colorMap.get(languages[i].name) ?? '#a6adb8'}
					stop-opacity="0.6"
				/>
				<stop
					offset="100%"
					stop-color={colorMap.get(languages[i1].name) ?? '#a7aeb9'}
					stop-opacity="0.6"
				/>
			</linearGradient>
		{/each}
	...
	<g>
		{#each languages as lang, i (lang.name)}
			...
			{@const c2y = p2[1] - (p3[1] - p1[1]) / 6}
			<path
				d={`M ${cx} ${cy} L ${p1[0]} ${p1[1]} C ${c1x} ${c1y} ${c2x} ${c2y} ${p2[0]} ${p2[1]} Z`}
				fill={`url(#gradFill${i})`}
				stroke="none"
			/>
		{/each}
